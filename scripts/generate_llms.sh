...
# CLIX React Native LLMS generator (outputs llms.txt)
# Usage:
...
#   EXCLUDE_DIRS    pruned dirs (default: node_modules coverage dist lib build .git .gradle .idea .vscode .swiftpm .github ios/Pods android/build .next)
#   EXTENSIONS      file extensions (default: ts,tsx,js,jsx,kt,java,swift,m,mm)
...
DEFAULT_EXT="ts,tsx,js,jsx,kt,java,swift,m,mm"

# Auto-detect INCLUDE_DIRS if not provided for React Native library repo
# Priority:
#  1) Known roots if present: src, android, ios, samples, example
...
if [[ -z "${INCLUDE_DIRS:-}" ]]; then
  dirs=()
  [[ -d "src" ]] && dirs+=("src")
  [[ -d "android" ]] && dirs+=("android")
  [[ -d "ios" ]] && dirs+=("ios")
  [[ -d "samples" ]] && dirs+=("samples")
  [[ -d "example" ]] && dirs+=("example")
  # include any new top-level directories with target source files
  while IFS= read -r topdir; do
    [[ "$topdir" =~ ^(\.git|\.github|\.vscode|node_modules|coverage|dist|lib|build|\.dart_tool|ios/Pods|android/build|gradle|.swiftpm)$ ]] && continue
    if find "$topdir" -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" -o -name "*.kt" -o -name "*.java" -o -name "*.swift" -o -name "*.m" -o -name "*.mm" \) -print -quit >/dev/null 2>&1; then
      dirs+=("$topdir")
    fi
  ...
  [[ -f "tsconfig.build.json" ]] && dirs+=("tsconfig.build.json")
...
EXCLUDE_DIRS="${EXCLUDE_DIRS:-node_modules coverage dist lib build .git .gradle .idea .vscode .swiftpm .github ios/Pods android/build .next}"
...
# Human-readable platform for header (fixed: React Native)
human_platform="React Native"
